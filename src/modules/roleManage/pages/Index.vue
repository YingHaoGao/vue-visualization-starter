<template>
<section class="lx-container" style="background:#FFF;padding: 20px;">
  <div class="filter" :class="{opened:!opened}">
    <el-row :gutter="20">
      <el-col :span="opened?24:10">
        <el-input v-model="filter.keyWord" style="margin-bottom:10px;" placeholder="请输入角色名称或编码"
      ...
      <el-col :span="opened?12:6" style="text-align:right">
        <el-button type="primary" round @click="fetchRoles" :loading="loading">
...
export default {
  ...
  mounted() {
    this.scrollToTop = animated => scrollToTop(document.body, animated);
  },
  methods: {
    fetchRoles() {
      this.loading = true;
      api.getRoles(this.filter).then(
        ({ data }) => {
